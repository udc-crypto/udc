...
public:
    ...
    explicit cdatastreams(int nTypeIn, int nVersionIn)
    {
        Init(nTypeIn, nVersionIn);
    }

    cdatastreams(const_iterator pbegin, const_iterator pend, int nTypeIn, int nVersionIn) : vch(pbegin, pend)
    {
        Init(nTypeIn, nVersionIn);
    }

#if !defined(_MSC_VER) || _MSC_VER >= 1300
    cdatastreams(const char* pbegin, const char* pend, int nTypeIn, int nVersionIn) : vch(pbegin, pend)
    {
        Init(nTypeIn, nVersionIn);
    }
#endif

    cdatastreams(const vector_type& vchIn, int nTypeIn, int nVersionIn) : vch(vchIn.begin(), vchIn.end())
    {
        Init(nTypeIn, nVersionIn);
    }

    cdatastreams(const std::vector<char>& vchIn, int nTypeIn, int nVersionIn) : vch(vchIn.begin(), vchIn.end())
    {
        Init(nTypeIn, nVersionIn);
    }

    cdatastreams(const std::vector<unsigned char>& vchIn, int nTypeIn, int nVersionIn) : vch((char*)&vchIn.begin()[0], (char*)&vchIn.end()[0])
    {
        Init(nTypeIn, nVersionIn);
    }
    ...
    cdatastreams& operator+=(const cdatastreams& b)
    {
        vch.insert(vch.end(), b.begin(), b.end());
        return *this;
    }

    friend cdatastreams operator+(const cdatastreams& a, const cdatastreams& b)
    {
        cdatastreams ret = a;
        ret += b;
        return (ret);
    ...
    short exceptions(short mask) { short prev = exceptmask; exceptmask = mask; setstate(0, "cdatastreams"); return prev; }
    cdatastreams* rdbuf()         { return this; }
    ...
    cdatastreams& read(char* pch, int nSize)
    {
        ...
        if (nReadPosNext >= vch.size())
        {
            if (nReadPosNext > vch.size())
            {
                setstate(std::ios::failbit, "cdatastreams::read() : end of data");
                memset(pch, 0, nSize);
    ...
    cdatastreams& ignore(int nSize)
    {
        ...
        {
            if (nReadPosNext > vch.size())
                setstate(std::ios::failbit, "cdatastreams::ignore() : end of data");
            nReadPos = 0;
            vch.clear();
            return (*this);
        }
    ...
    cdatastreams& write(const char* pch, int nSize)
    {
    ...
    template<typename T>
    cdatastreams& operator<<(const T& obj)
    {
