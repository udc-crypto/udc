...
protected:
    ...
    // delete for it.
    bool ScanBatch(const cdatastreams &key, std::string *value, bool *deleted) const;
    ...
    bool Read(const K& key, T& value)
    {
        ...
        // Unserialize value
        try {
            cdatastreams ssValue(strValue.data(), strValue.data() + strValue.size(),
...
public:
    ...
    bool AddTxIndex(const createnewtx& tx, const CDiskTxPos& pos, int nHeight);
    bool EraseTxIndex(const createnewtx& tx);
    bool ContainsTx(uint256 hash);
    bool ReadDiskTx(uint256 hash, createnewtx& tx, CTxIndex& txindex);
    bool ReadDiskTx(uint256 hash, createnewtx& tx);
    bool ReadDiskTx(COutPoint outpoint, createnewtx& tx, CTxIndex& txindex);
    bool ReadDiskTx(COutPoint outpoint, createnewtx& tx);
    bool WriteBlockIndex(const CDiskBlockIndex& blockindex);
    bool ReadHashBestChain(uint256& hashBestChain);
    bool WriteHashBestChain(uint256 hashBestChain);
    bool ReadBestInvalidTrust(nMaxLimit& bnBestInvalidTrust);
    bool WriteBestInvalidTrust(nMaxLimit bnBestInvalidTrust);
