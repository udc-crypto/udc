...
public:
    ...
    bool AddToWalletIfInvolvingMe(const createnewtx& tx, const PoWCompBlk* pblock, bool fUpdate = false, bool fFindBlock = false);
    bool EraseFromWallet(uint256 hash);
    void WalletUpdateSpent(const createnewtx& prevout, bool fBlock = false);
    int ScanForWalletTransactions(PoWCompBlkIndex* pindexStart, bool fUpdate = false);
    ...
    bool CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, createnewtx& txNew, CKey& key);
    ...
    bool IsMine(const createnewtx& tx) const
    {
    ...
    bool IsFromMe(const createnewtx& tx) const
    {
        return (GetDebit(tx) > 0);
    }
    int64_t GetDebit(const createnewtx& tx) const
    {
        int64_t nDebit = 0;
    ...
    int64_t GetCredit(const createnewtx& tx) const
    {
        int64_t nCredit = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
    ...
    int64_t GetChange(const createnewtx& tx) const
    {
        int64_t nChange = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
    ...
    void SetBestChain(const PoWCompBlkLocator& loc);
    ...
    void PrintWallet(const PoWCompBlk& block);
    ...
    void DisableTransaction(const createnewtx &tx);
...
public:
    ...
    CWalletTx(const CWallet* pwalletIn, const createnewtx& txIn) : CMerkleTx(txIn)
    {
        Init(pwalletIn);
    }
...
public:
    ...
    (
        ...
        if (!fRead)
        {
            ...
            {
                cdatastreams ss(nType, nVersion);
        ...
        if (fRead)
        {
            ...
            {
                cdatastreams ss(std::vector<char>(strComment.begin() + nSepPos + 1, strComment.end()), nType, nVersion);
                ss >> me.mapValue;
