...
BOOST_AUTO_TEST_CASE(tx_valid)
{
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        if (test[0].type() == array_type)
        {
            ...
            cdatastreams stream(ParseHex(transaction), SER_NETWORK, PROTOCOL_VERSION);
            createnewtx tx;
            stream >> tx;
...
BOOST_AUTO_TEST_CASE(basic_transaction_tests)
{
    ...
    cdatastreams stream(vch, SER_DISK, CLIENT_VERSION);
    createnewtx tx;
    stream >> tx;
...
//
static std::vector<createnewtx>
SetupDummyInputs(CBasicKeyStore& keystoreRet, MapPrevTx& inputsRet)
{
    std::vector<createnewtx> dummyTransactions;
...
BOOST_AUTO_TEST_CASE(test_Get)
{
    ...
    std::vector<createnewtx> dummyTransactions = SetupDummyInputs(keystore, dummyInputs);
