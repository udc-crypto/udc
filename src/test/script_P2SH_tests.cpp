...
BOOST_AUTO_TEST_CASE(sign)
{
    ...
    createnewtx txFrom;  // Funding transaction:
    ...
    createnewtx txTo[8]; // Spending transactions
    for (int i = 0; i < 8; i++)
    {
        txTo[i].vin.resize(1);
...
BOOST_AUTO_TEST_CASE(set)
{
    ...
    createnewtx txTo[4]; // Spending transactions
    for (int i = 0; i < 4; i++)
    {
        txTo[i].vin.resize(1);
        txTo[i].vout.resize(1);
...
{
    std::map<uint256, std::pair<CTxIndex, createnewtx> > mapInputs;
    CBasicKeyStore keystore;
    CKey key[3];
    vector<CKey> keys;
    for (int i = 0; i < 3; i++)
