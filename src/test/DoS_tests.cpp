...
extern bool AddOrphanTx(const cdatastreams& vMsg);
extern unsigned int LimitOrphanTxSize(unsigned int nMaxOrphans);
extern std::map<uint256, cdatastreams*> mapOrphanTransactions;
extern std::map<uint256, std::map<uint256, cdatastreams*> > mapOrphanTransactionsByPrev;
...
createnewtx RandomOrphan()
{
    std::map<uint256, cdatastreams*>::iterator it;
    it = mapOrphanTransactions.lower_bound(GetRandHash());
    if (it == mapOrphanTransactions.end())
        it = mapOrphanTransactions.begin();
    const cdatastreams* pvMsg = it->second;
    createnewtx tx;
    cdatastreams(*pvMsg) >> tx;
    return tx;
}
...
BOOST_AUTO_TEST_CASE(DoS_mapOrphans)
{
    ...
    for (int i = 0; i < 50; i++)
    {
        ...
        cdatastreams ds(SER_DISK, CLIENT_VERSION);
        ds << tx;
        AddOrphanTx(ds);
    }
    ...
    for (int i = 0; i < 50; i++)
    {
        createnewtx txPrev = RandomOrphan();
...
BOOST_AUTO_TEST_CASE(DoS_checkSig)
{
    ...
    createnewtx orphans[NPREV];
    for (int i = 0; i < NPREV; i++)
    {
        createnewtx& tx = orphans[i];
        tx.vin.resize(1);
        tx.vin[0].prevout.n = 0;
        tx.vin[0].prevout.hash = GetRandHash();
        tx.vin[0].scriptSig << OP_1;
