...
void CWallet::SetBestChain(const PoWCompBlkLocator& loc)
{
...
void CWallet::WalletUpdateSpent(const createnewtx &tx, bool fBlock)
{
...
bool CWallet::AddToWalletIfInvolvingMe(const createnewtx& tx, const PoWCompBlk* pblock, bool fUpdate, bool fFindBlock)
{
    uint256 hash = tx.GetHash();
    {
        LOCK(cs_wallet);
...
int CWallet::ScanForWalletTransactions(PoWCompBlkIndex* pindexStart, bool fUpdate)
{
    int ret = 0;

    PoWCompBlkIndex* pindex = pindexStart;
    {
        ...
        while (pindex)
        {
            ...
            BOOST_FOREACH(createnewtx& tx, block.vtx)
            {
    ...
    BOOST_FOREACH(const CMerkleTx& tx, vtxPrev)
    {
        if (!(tx.IsCoinBase() || tx.IsCoinStake()))
        {
            ...
            if (!txdb.ContainsTx(hash))
                RelayTransaction((createnewtx)tx, hash);
        }
    }
    if (!(IsCoinBase() || IsCoinStake()))
    {
        ...
        if (!txdb.ContainsTx(hash))
        {
            ...
            RelayTransaction((createnewtx)*this, hash);
        }
    }
}
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                // Limit size
                unsigned int nBytes = ::GetSerializeSize(*(createnewtx*)&wtxNew, SER_NETWORK, PROTOCOL_VERSION);
...
bool CWallet::GetStakeWeight(const CKeyStore& keystore, uint64_t& nMinWeight, uint64_t& nMaxWeight, uint64_t& nWeight)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        nMaxLimit bnCoinDayWeight = nMaxLimit(pcoin.first->vout[pcoin.second].nValue) * nTimeWeight / COIN / (24 * 60 * 60);
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, createnewtx& txNew, CKey& key)
{
...
void CWallet::PrintWallet(const PoWCompBlk& block)
{
    {
        LOCK(cs_wallet);
...
void CWallet::DisableTransaction(const createnewtx &tx)
{
...
void CWallet::GetKeyBirthTimes(std::map<CKeyID, int64_t> &mapKeyBirth) const {
    ...
    PoWCompBlkIndex *pindexMax = FindBlockByHeight(std::max(0, nBestHeight - 144)); // the tip can be reorganised; use a 144-block safety margin
    std::map<CKeyID, PoWCompBlkIndex*> mapKeyFirstBlock;
    ...
    for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); it++) {
        ...
        std::map<uint256, PoWCompBlkIndex*>::const_iterator blit = mapBlockIndex.find(wtx.hashBlock);
        if (blit != mapBlockIndex.end() && blit->second->IsInMainChain()) {
            ...
            BOOST_FOREACH(const CTxOut &txout, wtx.vout) {
                ...
                BOOST_FOREACH(const CKeyID &keyid, vAffected) {
                    ...
                    std::map<CKeyID, PoWCompBlkIndex*>::iterator rit = mapKeyFirstBlock.find(keyid);
    ...
    for (std::map<CKeyID, PoWCompBlkIndex*>::const_iterator it = mapKeyFirstBlock.begin(); it != mapKeyFirstBlock.end(); it++)
